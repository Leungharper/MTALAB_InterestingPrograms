    ...
    for i = 1-delta_y:height-delta_y                        %从变换图像中反向寻找原图像的点，以免出现空洞，和旋转放大原理一样
        for j = 1-delta_x:width-delta_x
            ...
            if pix(1)>=0.5 && pix(2)>=0.5 && pix(1)<=M && pix(2)<=N
                imgn(i+delta_y,j+delta_x,k)=img(round(pix(1)),round(pix(2)),k);     %最邻近插值,也可以用双线性或双立方插值
            end
        end
    end
end
...
imwrite(uint8(imgn) , [filename,'.png']);
toc
